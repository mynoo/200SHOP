...
public class ReplyUpdateController extends SuperClass {
	...
	public void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		Reply bean01 = dao.SelectDataByPk(no) ; 
		
		request.setAttribute("bean01", bean01);
		
		String gotopage = "/reply/reply.jsp" ;
	...
	public void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		if (this.validate(request) == true) {
			System.out.println("reply update validation check success");
			ReplyDao dao = new ReplyDao();
			int cnt = - 999999 ;
			cnt = dao.UpdateData(bean01) ;
			
			
		} else {
			System.out.println("reply update validation check failure");
