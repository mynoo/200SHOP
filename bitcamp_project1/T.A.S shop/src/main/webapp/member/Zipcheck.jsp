...
<head>
	...
	<script type="text/javascript">
		function sendAddress(zipcode, address){
			opener.minsertform.fakezipcode.value = zipcode ;
			opener.minsertform.zipcode.value = zipcode ;
			
			opener.minsertform.fakeaddress1.value = address ;
			opener.minsertform.address1.value = address ;
			
			opener.minsertform.address2.focus() ;
			self.close() ;
		}
		function myzipcheck(){
			var dong = document.minsertform.dong.value ;
			if(dong.length <= 1){
				alert('동네 이름은 2글자 이상 입력하셔야 합니다.');
				document.minsertform.dong.focus() ;
				return false ; 
			}
		}
	</script>
